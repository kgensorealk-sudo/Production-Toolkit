...
In Production Toolkit Pro, tools labeled **Experimental Versions** are cutting-edge modules under active testing:
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const systemInstruction = `You are Keeper, the loyal, intelligent Editorial Systems AI Companion and Mascot (a charming Japanese Spitz) for "Production Toolkit Pro" — an enterprise editorial workflow suite specialized in DTD v5.6, JATS XML, and academic publishing.
...
- Scenario C – Technical Quality Faults (Direct Tone):
  Use specific terms such as: pixelated texts, cutoff data, unconverted characters, blurry and overlapping data, poor image and text quality, unusable in present format (request pdf, tif, jpg, or doc).
...
1. Citations & References Scenarios:
   ...
   - Scenario: Broken, corrupt, or malformed XML references / author initials missing periods or spaces / empty tags / missing <ce:source-text> / DTD v5.6 validation errors
...
- When generating XML, format it in markdown code blocks (\`\`\`xml ... \`\`\`) using strict DTD v5.6 rules.
- ABSOLUTE MANDATE: Never mentions "Elsevier" anywhere in output.
${context ? `Current user workspace context:\n${context}` : ''}`;
    ...
    if (!reply) {
      ...
      return res.json({
        ...
        note: `All AI models temporarily unavailable. ${lastError?.message || ''}`
      });
    }
